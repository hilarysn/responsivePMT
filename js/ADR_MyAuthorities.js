...
CKO.DASHBOARDS.ALLDASHBOARDS.Authorities = function () {
    ...
    function GetActions() {
        ...
        if (v.url === null) {
            ...
            urlString += "$select=Id,Title,Expended,PMTUser/Id,DateCompleted,EffortTypeValue,Function";
            var today = new Date();                                                                                                                                             //
        ...
        logit("My Authorities Chart Query urlString: " + v.url);
                        ...
                        for (var k = 0; k < v.standards.length; k++) {
                            if (j[i]["Title"] === v.standards[k].title) {
                                ...
                                for (var m = 0; m < v.authorities.length; m++) {
                                    if (v.authorities[m].auth === aut && j[i]["Expended"] !== 0) {
