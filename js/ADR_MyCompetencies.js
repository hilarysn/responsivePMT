...
CKO.DASHBOARDS.ALLDASHBOARDS.Competencies = function () {
    ...
    function LoadFunctions() {
        ...
        deferreds.push($.when(CKO.REST.GetListItems.getitems(urlString)).then(function (data) {
            ...
            for (var i = 0, length = j.length; i < length; i++) {
                ...
                logit("Functions list for Competencies data push: " + data);
            }
        }, function (data) {
            $("#SPSTools_Notify").fadeOut("2500", function () {
                $("#SPSTools_Notify").html("");
            });
            logit("Functions list for Competencies data fade: " + data);
        }));
        return deferreds;
    }
    ...
    function ActionsLoaded() {
        ...
        if (v.url === null) {
            ...
            urlString += "$select=Id,Expended,DateCompleted,PMTUser/Id,Function";
            var today = new Date();                                                                                                                                             //
        ...
        logit("My Comptencies chart V.URL: " + v.url);
